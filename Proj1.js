...
(function(arrOfTasks) {
  ...
  listContainer.addEventListener('click', executeTaskHendler)
    ...
    if (!tasksList) {
      ...
      Object.values(tasksList).forEach(task => {
        if (task.completed == true) {
          fragment.append(listItemTemplate(task));
        } else {
          fragment.prepend(listItemTemplate(task));
        }
      });
    ...
    if (eventBtn.classList.contains('unfinished-tasks')) {
      ...
      if (eventBtn.classList.contains('completed-tasks')) {
        ...
        if (eventBtn.classList.contains('all-tasks')) {
          ...
          const ul = listContainer.children;
          [...ul].forEach(item => {
            item.remove();
          });
  ...
  function boolComolited(bool) {
    const newArrTasks = arrOfTasks.filter(item => item.completed == bool);
    const ul = listContainer.children;
    [...ul].forEach(item => {
      item.remove();
    });
  ...
  function deleteTask(id) {
    ...
    const isConfirm = confirm(`Вы увурены что хотите удалить задачу? ${title}`);
    if (!isConfirm) return isConfirm;
    delete arrOfTasks[id];
    return isConfirm;
  }
  ...
  function complitedObjectTask(id, bool) {
    const obj = objOfTasks[id];
    // const i = arrOfTasks.indexOf(obj, 0);
    objOfTasks[id].completed = bool;
    // console.log(i)
    // arrOfTasks[i].completed = bool;
  }

  function executeTaskHendler(event) {
    const executeBtn = event.target;
    const parentExecute = executeBtn.parentElement;
    const id = parentExecute.getAttribute('data-task-id');
    if (executeBtn.classList.contains('completed-btn')) {
      parentExecute.style.background = '#D3D3D3';
      listContainer.insertAdjacentElement('beforeend', parentExecute);
      ...
      if (executeBtn.classList.contains('reject-btn')) {
        ...
        listContainer.insertAdjacentElement('afterbegin', parentExecute);
